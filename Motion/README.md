# Zero-shot Guidance
After preparing the project with the following instructions. The command to run zero-shot guidance is 
'''python -m sample.generate --model_path {check_point_path} --text_prompt {prompt} --num_repetitions 3 --goal {goal position} --method {FreeDoM or DPS or PGD} --seed {seed} --rho {step_size}'''

The corresponding commands for the improved methods (ours) are:
'''python -m sample.generate --model_path ./save/humanml_trans_enc_512/model000200000.pt --text_prompt "the person is walking." --num_repetitions 5 --goal 6 6 --method PGD --rho 1'''
'''python -m sample.generate --model_path ./save/humanml_trans_enc_512/model000200000.pt --text_prompt "the person is jogging." --num_repetitions 5 --goal 8 8 --method PGD --rho 2'''

Note: The baseline FreeDoM in this code is slightly different from the one used in the paper. FreeDoM used in the paper is with more resampling steps and a better performance.
