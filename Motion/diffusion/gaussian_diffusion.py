...
class GaussianDiffusion:
    ...
    def decode_xyz(self, sample):
        n_joints = 22 if sample.shape[1] == 263 else 21
    
        sample = global_vars.data_backup.inv_transform_gpu(sample.permute(0, 2, 3, 1))
        sample = motion_process.recover_from_ric(sample, n_joints)
        sample = sample.view(-1, *sample.shape[2:]).permute(0, 2, 3, 1)
        return sample

    '''def collision(self, x_0, x_t, target, obs, noise_scale = None):
        def collision_loss(x_0, y_obs):
            y_obs = torch.tensor(y_obs, device=x0.device).unsqueeze(-1).unsqueeze(0)
            return (F.sigmoid(-(torch.norm(x_0 - y_obs, dim = [1]) - 1.2) * 50) * 100).mean()
        ...
        x0 = self.decode_xyz(x_0)
        x0_aug = x0.repeat((10,1,1,1))
        x0_aug += noise_scale * torch.randn_like(x0_aug, device = x0_aug.device)
        xy = torch.cat([x0_aug[:,:,0:1,:], x0_aug[:,:,2:3,:]], axis = 2).mean(dim = [1])
        target_loss = torch.norm(x0_aug[:,:,0,-1].mean(dim = [1]) - x_target) ** 2 + torch.norm(x0_aug[:,:,2,-1].mean(dim = [1]) - y_target) ** 2
        obs_loss = collision_loss(xy, obs)
        obj = target_loss + obs_loss
    
    
        return torch.autograd.grad(outputs=obj, inputs=x_t)[0]'''

    def collision(self, x_0, x_t, target, obs, loss = 'dps', noise_scale = None, out = True):
        def collision_loss(x_0, y_obs):
            y_obs = torch.tensor(y_obs, device=x0.device).unsqueeze(-1).unsqueeze(0)
            return (torch.sigmoid(-(torch.norm(x_0 - y_obs, dim = [1]) - (global_vars.radius + 0.2) ) * 50) * 100).mean()
        ...
        target_loss = torch.norm(x0[:,:,0,-1].mean(dim = [0,1]) - x_target) ** 2 + torch.norm(x0[:,:,2,-1].mean(dim = [0,1]) - y_target) ** 2
        xy = torch.cat([x0[:,:,0:1,:], x0[:,:,2:3,:]], axis = 2).mean(dim = [1])
        obs_loss = collision_loss(xy, obs)
        obj = target_loss + obs_loss
    ...
    def ddim_sample(
        ...
        with torch.enable_grad():
            ...
            grad = self.collision(out["pred_xstart"], x_input, global_vars.goal, (global_vars.goal[0]/2, global_vars.goal[1]/2), noise_scale = 0)

        if global_vars.method == 'PGD':
            sample = mean_pred + nonzero_mask * sigma * noise - global_vars.rho * grad / torch.norm(grad)
        elif global_vars.method == 'FreeDoM':
            sample = mean_pred + nonzero_mask * sigma * noise - global_vars.rho * grad * alpha_bar.sqrt()
        elif global_vars.method == 'DPS':
            sample = mean_pred + nonzero_mask * sigma * noise - global_vars.rho * grad
